...
def create_api_key(api_key: api_schema.APIKeyCreate, db: Session = Depends(get_db)):
...
def get_api_keys(db: Session = Depends(get_db)):
    return api_service.get_api_keys(db)
...
def update_api_key(key_id: int, api_key_data: api_schema.APIKeyCreate, db: Session = Depends(get_db)):
